...
void ListComponent::onUpdateProperties(const nlohmann::json& properties) {
    ...
    // Match Android ListComponent behavior:
    // - vertical list: width = MATCH_PARENT (100%), height = WRAP_CONTENT (Yoga)
    // - horizontal list: width = WRAP_CONTENT (Yoga), height = MATCH_PARENT (100%)
    ...
    if (!styleInfo.empty()) {
        try {
            ...
            userSetHeight = styleInfoJson.contains("height");
    ...
    A2UIListNode node(m_nodeHandle);
    // Vertical list width is already set to 100% in the constructor.
    // Horizontal list height needs to be set here.
    if (!userSetHeight && m_horizontal) {
        node.setPercentHeight(1.0f);
    }
    // Sync ListItem cross-axis size.
    ...
    if (!userSetHeight && m_horizontal) {
        for (auto& pair : m_listItemWrappers) {
            A2UINode(pair.second).setPercentHeight(1.0f);
        }
    }
...
void ListComponent::updateListItemMargins() {
    ...
    for (size_t i = 0; i < m_children.size(); ++i) {
        ...
        if (i == 0) {
            // First item: margin = Yoga y (includes list padding-top).
        ...
        A2UINode(listItemHandle).setMargin(marginTop, 0.0f, 0.0f, 0.0f);
    }
}
...
ArkUI_NodeHandle ListComponent::createListItemWrapper(ArkUI_NodeHandle childHandle) {
    ...
    if (m_horizontal) {
        A2UINode(listItemHandle).setPercentHeight(1.0f);
    } else {
