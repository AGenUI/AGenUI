...
bool TextComponentMeasurement::buildParam(const nlohmann::json& j,
    ...
    if (fwVal.is_number()) {
        ...
        const std::string fw = fwVal.get<std::string>();
        if      (fw == "bold")   outParam.fontWeight = NODE_PROPERTY_FONT_BOLD;
        else if (fw == "bolder") outParam.fontWeight = NODE_PROPERTY_FONT_BOLDER;
        else if (fw == "normal") outParam.fontWeight = NODE_PROPERTY_FONT_NORMAL;
        else                     outParam.fontWeight = std::atoi(fw.c_str());
    }
    ...
    float lhFloat = 0.0f;
    if (lhVal.is_number()) {
        lhFloat = static_cast<float>(lhVal.get<double>());
    } else if (lhVal.is_string()) {
        lhFloat = static_cast<float>(std::atof(lhVal.get<std::string>().c_str()));
    }
    if (lhFloat > 0.0f) {
        outParam.lineHeight = (lhFloat < 5.0f) ? lhFloat
                                                : lhFloat / static_cast<float>(outParam.fontSize);
    }
