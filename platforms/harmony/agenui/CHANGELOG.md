...
- **(鸿蒙) `line-height` 测量重构**：测量改为携带解析后的绝对行盒高度，与渲染侧完全一致。显式设置的 `line-height` 即使小于字体自然行高也一样定义行盒，`white-space` 不再影响行高解析。
- **(鸿蒙) Playground 发布构建脚本**：新增鸿蒙 Playground 发布构建脚本，使用发布证书与 profile 签名，产出可上架应用市场的 `.app` 包。
- **`font-weight` 能力对齐**：三端统一支持 `normal` / `medium` / `bold` 关键字与 100～900 数值字重（可写成 JSON 数字或整数字符串），其他取值回退为 400。catalog 中同步补充了平台差异说明与 `font-family` 描述。
- **(iOS) 运行时配置接口**：新增 `AGenUI.setRuntimeConfig(_:)`，接收宿主下发的 JSON 开关串。需在 SDK 初始化阶段、任何 Surface 渲染之前调用一次，该值在会话内固定。
- **(iOS) 物理像素对齐**：细线边框提升为整数物理像素宽度，使四条边在各种屏幕倍率下粗细一致；组件 frame 对齐到像素网格，使相邻兄弟节点之间不留缝隙。由 `borderPixelAlignment` 开关控制（默认开启）。

### Bug 修复

- (鸿蒙) 修复 DateTimeInput 测量时携带残留行高覆盖值的问题。
- (Android) 修复特定屏幕密度下不可断行的数字与中文字符串整体消失的问题。测量不再使用 `linearText`，其亚像素度量窄于渲染路径，会让 Yoga 的测量缓存误判文字实际会折行的宽度为有效。
