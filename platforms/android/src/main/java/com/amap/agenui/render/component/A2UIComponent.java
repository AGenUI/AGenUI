...
public abstract class A2UIComponent {
    ...
    protected final boolean reportRenderSizeIfNeeded(View targetView,
        ...
        if (widthPx <= 0) {
            Log.d(logTag, "⏭ [RENDER_FINISH] Skip report for " + id + ", unresolved width");
            return false;
        }
        ...
        Log.d(logTag, "📏 [RENDER_FINISH] " + type + " " + id
    ...
    private int resolveWidthFromStylesPx(View targetView, Map<String, Object> propertySource) {
        ...
        if (widthValue instanceof Number) {
            return Math.max(0, Math.round(StyleHelper.a2uiToPx(targetView.getContext(), ((Number) widthValue).floatValue())));
        }
    ...
    private void applyYogaLayout(View targetView,
        ...
        if (surfaceLayoutDispatcher != null && allowDispatch) {
            surfaceLayoutDispatcher.dispatchLayout(targetView, parent, nextLayout);
        } else {
    ...
    private YogaAbsoluteLayout.LayoutState buildYogaLayout(Map<String, Object> styles, View targetView) {
        ...
        int xPx = Math.round(StyleHelper.a2uiToPx(targetView.getContext(), readA2uiFloat(styles.get("x"))));
        int yPx = Math.round(StyleHelper.a2uiToPx(targetView.getContext(), readA2uiFloat(styles.get("y"))));
        int widthPx = Math.max(0, Math.round(StyleHelper.a2uiToPx(targetView.getContext(), readA2uiFloat(styles.get("width")))));
        int heightPx = Math.max(0, Math.round(StyleHelper.a2uiToPx(targetView.getContext(), readA2uiFloat(styles.get("height")))));
