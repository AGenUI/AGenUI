...
final class TextMeasurer {
    ...
    private static MeasureResult measureTextValue(Context context,
        ...
        try {
            ...
            int lineCount = 0;
            if (layoutWidthPx > 0) {
                Layout layout = buildLayout(displayText, layoutStyle, layoutWidthPx);
            ...
            return MeasureResult.sync(
                    StyleHelper.pxToA2ui(context, measuredWidthPx),
                    StyleHelper.pxToA2ui(context, measuredHeightPx),
                    lineCount);
        } catch (Exception ignored) {
            return MeasureResult.zero();
        }
            ...
            if (lineHeightValue != null) {
                ...
                if (lineHeightStr.matches("^\\d+(\\.\\d+)?$")) {
                    ...
                    if (parsedMultiplier != null && parsedMultiplier > 0f) {
                        lineSpacingMultiplier = parsedMultiplier;
                    }
                } else if (lineHeightStr.endsWith("px")) {
                    int lineHeightPx = StyleHelper.parseDimension(lineHeightValue, context);
                    if (lineHeightPx > 0) {
                        lineSpacingAddPx = lineHeightPx - paint.getTextSize();
    ...
    private static Typeface resolveTypeface(Context context, JSONObject styles) {
        Typeface baseTypeface = StyleHelper.parseFontFamily(
                ...
                context);
        boolean bold = "bold".equalsIgnoreCase(String.valueOf(firstStyleValue(styles, "font-weight", "fontWeight")));
    ...
    private static int resolveConstraintPx(Context context, float maxSize) {
        ...
        return Math.max(Math.round(StyleHelper.a2uiToPx(context, maxSize)), 0);
    }
