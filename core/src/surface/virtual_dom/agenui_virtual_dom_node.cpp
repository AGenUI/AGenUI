...
VirtualDOMNode::VirtualDOMNode(const std::string& id,
    ...
    , _yogaNodeManager(yogaNodeManager)
{
    if (_yogaNodeManager) {
        _yogaNode = _yogaNodeManager->createNode(_id);
    }
}

VirtualDOMNode::~VirtualDOMNode() {
    notifyComponentRemoved();
    // YogaNode lifecycle is managed by YogaNodeManager.
    // Tree destruction order: parent _children vector destructs first -> child destructs.
    // YogaNode::~YogaNode already handles _hasOwner detach; no need to call removeNode here.
    // Just null the raw pointer to avoid dangling.
    _yogaNode = nullptr;
    _yogaNodeManager = nullptr;
}
...
void VirtualDOMNode::logYogaLayoutInfo(const ComponentSnapshot& snapshotWithLayout) const {
    ...
    AGENUI_LOG("  layout: x=%.1f y=%.1f w=%.1f h=%.1f flexGrow=%.1f flexShrink=%.1f childCount=%u aspectRatio=%.4f hasMeasureFunc=%d",
...
void VirtualDOMNode::setupMeasureFunctionIfNeeded() {
    if (!_snapshot || !_yogaNode) {
        return;
    }

    const std::string& comp = _snapshot->component;

    // Route measurement via _measurementManager: if registered, set measureFunc; otherwise do nothing
    if (!_measurementManager) {
        return;
    }
    // If the platform has already supplied a concrete size (via notifyRenderFinish → setYogaNodeSize),
    // skip re-registering the measureFunc. It will be re-enabled when the snapshot content changes.
    if (_hasPlatformSize) {
        _yogaNode->clearMeasureFunc();
        return;
    }
    if (_measurementManager->getMeasurement(comp)) {
        // Yoga rule: nodes with children cannot also have a measureFunc (mutually exclusive).
        // But special container components (allowsMeasureWithChildren()==true, e.g. Tabs)
        // need measureFunc even when they have Yoga children.
        auto impl = _measurementManager->getMeasurement(comp);
        const bool hasChildren = _yogaNode->childCount() > 0;
        if (hasChildren && !impl->allowsMeasureWithChildren()) {
            // Has children and not allowed to set measureFunc: clear any residual measureFunc
            _yogaNode->clearMeasureFunc();
            return;
        }

        // If both width and height are explicit pixel values, Yoga can compute the size
        // directly from styles without calling measureFunc. Registering measureFunc in this
        // case is wasteful and can cause oscillation loops (e.g. Image 720px vs 718px).
        auto isExplicitPx = [](const std::string& val) -> bool {
        ...
        if (isExplicitPx(getStyleStr("width")) && isExplicitPx(getStyleStr("height"))) {
            _yogaNode->clearMeasureFunc();
            return;
        }

        // If aspect-ratio is set, Yoga can derive the missing dimension from the known one.
        // measureFunc must NOT be registered in this case: Yoga ignores aspect-ratio when
        // a measureFunc is present (the func's return value takes full precedence).
        const std::string aspectRatioStr = getStyleStr("aspect-ratio");
        AGENUI_LOG("[setupMeasureFunc] id=%s comp=%s aspect-ratio='%s' width='%s' height='%s'",
            _id.c_str(), comp.c_str(), aspectRatioStr.c_str(),
            getStyleStr("width").c_str(), getStyleStr("height").c_str());
        if (!aspectRatioStr.empty() && aspectRatioStr != "auto") {
            _yogaNode->clearMeasureFunc();
            AGENUI_LOG("[setupMeasureFunc] id=%s: aspect-ratio present, cleared measureFunc", _id.c_str());
            return;
        }

        // Register unified MeasureCallback via YogaNode::setMeasureFunc
...
void VirtualDOMNode::updateChildren() {
    ...
    for (; i < targetSize; i++) {
        ...
        while (i < _children.size() && targetId != _children[i]->getId()) {
            ...
            if (currentChild && _yogaNodeManager) {
                _yogaNodeManager->removeChild(_id, currentChild->getId());
            }
        ...
        if (i >= _children.size()) {
            ...
            if (_yogaNodeManager && newChild->_yogaNode && !newChild->_yogaNode->isAttached()) {
                _yogaNodeManager->insertChild(_id, targetId, static_cast<uint32_t>(i));
            }
        }
    ...
    while (i < _children.size()) {
        if (_children[i] && _yogaNodeManager) {
            _yogaNodeManager->removeChild(_id, _children[i]->getId());
        }
        _children.erase(_children.begin() + i);
    }
}
