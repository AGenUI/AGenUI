...
// Safely parse a CSS token that may carry a unit suffix (e.g. "8px", "1.5em").
// Strips any non-numeric trailing characters before calling stof.
// Returns 0.0f and sets ok=false when the numeric part is empty or conversion fails.
float parseCssFloat(const std::string& token, bool* ok = nullptr) {
    if (token.empty()) {
        if (ok) *ok = false;
        return 0.0f;
    }
    // Find first character that is not part of a number (digits, '.', '-', '+').
    size_t unitPos = token.find_first_not_of("0123456789.-+");
    const std::string numericPart = (unitPos != std::string::npos)
        ? token.substr(0, unitPos)
        : token;
    ...
    try {
        float result = std::stof(numericPart);
        if (ok) *ok = true;
        return result;
    } catch (...) {
        if (ok) *ok = false;
